A type alias—often called a typedef because it's declared with the `typedef` keyword—is a concise way to refer to a type. Here's an example of declaring and using a type alias named `IntList`:
